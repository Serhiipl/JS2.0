...
function f1() {
  ...
  document.querySelector(".out-1").textContent = el;
}
...
function f2() {
  ...
  document.querySelector(".out-2").textContent = el;
}
...
function f9() {
  let maxIndex = 0;
  for (const key in a9) {
    if (maxIndex < a9[key].length - 1) maxIndex = a9[key].length - 1;
  }
  return maxIndex;
}
